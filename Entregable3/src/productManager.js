...
export class ProductManager {
    ...
    async addProduct(obj) {
        ...
        let codeFlag = await this.existCode(obj.code)

        if (codeFlag === true) return "El código ya existe. Ingrese uno diferente para el nuevo producto";
        if (obj.title === undefined || obj.title === "") return "Por favor ingrese un nombre distinto de vacío";
        if (obj.description === undefined || obj.description === "") return "Por favor ingrese una descripción distinta de vacío";
        if (obj.price === undefined || obj.price <= 0) return "Por favor ingrese un precio mayor a $0";
        if (obj.stock === undefined || obj.stock <= 0) return "Por favor ingrese un stock mayor a 0";
