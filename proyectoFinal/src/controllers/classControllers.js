...
export default class Controllers {
    ...
    create = async (req, res, next) => {
        try {
            ...
            if (!newItem) createResponse(res, 404, { method: 'create', error: 'Error create item' })
    ...
    update = async (req, res, next) => {
        try {
            const { id } = req.params;
            const item = await this.service.getById(id);
            if (!item) createResponse(res, 404, { method: 'update', error: 'Error update item' })
    ...
    delete = async (req, res, next) => {
        try {
            const { id } = req.params;
            const item = await this.service.getById(id);
            if (!item) createResponse(res, 404, { method: 'delete', error: 'Error delete item' })
